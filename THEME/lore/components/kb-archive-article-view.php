...
<div class="page-kb-post page-kb-post-archive page-kb-post-archive-article-view">

	<?php // MAIN HEADER
	lsvr_lore_the_main_header(); ?>

	<?php // Add custom code before Knowledge Base archive
	do_action( 'lsvr_lore_kb_archive_article_view_before' ); ?>

	<?php if ( have_posts() ) : ?>

		<?php $post_count = wp_count_posts( 'lsvr_lore_kb' ); ?>
		<?php if ( ! empty( $post_count->publish ) ) : ?>
		<h6><?php echo sprintf( esc_html( _n( '%s article', '%s articles', $post_count->publish, 'lore' ) ), $post_count->publish ); ?></h6>
		<?php endif; ?>
	...
	do_action( 'lsvr_lore_kb_archive_article_view_after' ); ?>
